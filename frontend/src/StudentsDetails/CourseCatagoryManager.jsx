...
import { Plus, List, Loader2, Trash2, Edit, Save, FolderPlus, AlignLeft, Link as LinkIcon } from 'lucide-react';
  ...
  return (
    <div className="w-full font-sans">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* LEFT COMPACT FORM (Takes up 5 columns) */}
        <div className="lg:col-span-5 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm h-fit">
          <div className="flex justify-between items-center mb-4">
            ...
            {editingId && (
              <button onClick={() => { setEditingId(null); setFormData({name: "", description: ""}); }} className="text-xs font-semibold text-gray-500 hover:text-gray-800 bg-gray-100 px-2 py-1 rounded">Cancel</button>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input 
                type="text" required placeholder="e.g., Artificial Intelligence"
                value={formData.name}
            ...
            <div>
              ...
              <textarea 
                required rows="2" placeholder="Brief summary of this sector..."
              ...
              categories.map(cat => (
                <div key={cat._id} className="bg-white p-3.5 rounded-xl border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all group relative pr-10 flex flex-col justify-between">
                  ...
                  <div className="mt-3 pt-2 border-t border-gray-50">
                    <span className="text-[9px] font-mono bg-gray-50 text-gray-400 px-1.5 py-0.5 rounded inline-block">
                      /{cat.slug}
                    </span>
                  </div>
                  ...
                  <div className="absolute top-3 right-2 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEditClick(cat)} className="text-gray-400 hover:text-amber-500 hover:bg-amber-50 transition-colors p-1.5 rounded-lg border border-transparent hover:border-amber-100">
                      <Edit size={14} />
                    </button>
                    <button onClick={() => handleDelete(cat._id, cat.name)} className="text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors p-1.5 rounded-lg border border-transparent hover:border-red-100">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
